// Bugatti Veyron Chassis Car file.

[GENERAL]
//AdjustableSuspensionLevelStock=1
...
Mass=1600.0                      // all mass except fuel  (1800kg car + 80kg driver weight)
Inertia=(2380.0,2500.0,480.0)    // all inertia except fuel
FuelTankPos=(0.0, 0.200, -0.430)  // location of tank affects center of gravity (very close and even slightly ahead of CG in F1)
FuelTankMotion=(560.0,0.7)      // simple model of fuel movement in tank (spring rate per kg, critical damping ratio)
Notes=""
Symmetric=1
CGHeight=0.270                  // height of body mass (excluding fuel) above reference plane
...
Undertray00=(0.95,  0.20, -1.895)  // offset from center of car, and between axles// LF (Left front corner of splitter) (forward is negative, right is negative)
Undertray01=(-0.95, 0.20, -1.895)  // offset from center of car, and between axles// RF (Right front corner of splitter)
Undertray02=(0.35,  0.20, 1.000)  // offset from center of car, and between axles // LR (Left rear corner of floor
Undertray03=(-0.35, 0.20, 1.000)  // offset from center of car, and between axles // RR (Right rear corner of floor)
...
FWDragParams=(0.05, 0.005, 0.0001)        // base drag and 1st and 2nd order with setting
FWLiftParams=(-0.310, -0.019, 0.00002)  // base lift and 1st and 2nd order with setting
...
RWDragParams=(0.050, 0.007, 0.0001)        // base drag and 1st and 2nd order with setting
RWLiftParams=(-0.470, -0.024, 0.00002)  // base lift and 1st and 2nd order with setting
...
RWCenter=(0.00, 0.60, 0.30)     // center of rear wing forces (offset from center of rear axle at ref plane)
...
BodyUp=(0.0, -0.8, 0.0)         // aero forces from moving up
...
BodyRot=(6.00, 5.5, 3.60)         // aero torque from rotating 
...
BrakeDuctLift=(0.002)           // effect of brake duct setting on lift

[DIFFUSER]
DiffuserBase=(-0.400, -0.50, 5.0) // base lift and 1st/2nd order with rear ride height
DiffuserFrontHeight=(0.600)         // 1st order with front ride height
...
RearAntiSwayRange=(7500.0, 7500.0, 8)
...
DownshiftAlgorithm=(0.80,0.75,0.70) // percentage of "optimum" downshift point in high gears, percentage of "optimum" downshift point in low gears, oval adjustment.
...
Gear6Setting=58
Gear7Setting=60
DiffPumpTorque=800.0                // at 100% pump diff setting, the torque redirected per wheelspeed difference in radians/sec (roughly 1.2kph)
DiffPumpRange=(0.00, 0.05, 20)
DiffPumpSetting=10
DiffPowerRange=(0.0,0.00,0)        // fraction of power-side input torque transferred through diff
DiffPowerSetting=0                 // (not implemented for four-wheel drive)
DiffCoastRange=(0.0,0.00,0)        // fraction of coast-side input torque transferred through diff
...
SpinInertia=1.50                   // inertia in pitch direction including any axle
...
BrakeTorque=5000.0                  // maximum brake torque at zero wear and optimum temp
BrakeHeating=0.00032                 // heat added linearly with brake torque
...
SpinInertia=1.70                   // inertia in pitch direction including any axle
...
PackerSetting=0
SpringMult=1.0                  // take into account suspension motion if spring is not attached to spindle (affects physics but not garage display)
SpringRange=(95000.0, 7916.0, 6)
SpringSetting=0
RideHeightRange=(0.110, -0.005, 8)
RideHeightSetting=4
DamperMult=1.00                     // take into account suspension motion if damper is not attached to spindle (affects physics but not garage display)
SlowBumpRange=(4000.0, 300.0, 6)
SlowBumpSetting=0
...
BrakeHeating=0.00027                // heat added linearly with brake torque
